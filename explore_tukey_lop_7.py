...
def explore_tukey_lop(data, powers=[-2, -1, -0.5, 0, 0.5, 1, 2], BoxCox = True, normtest="K^2 test", outlierremoval=False):

    # %% Subfunction definitions
    def data_transform(data, tk_power):
        if tk_power != 0:
            if tk_power != 3:
                a = np.power(data.astype("float"), 2)
            else:
                data_min = np.nanmin(data)
                if data_min <= 0:
                    data = data - data_min + 1
                a, _ = boxcox(data.astype("float"))
        else:
            data_min = np.nanmin(data)
            if data_min <= 0:
                data = data - data_min + 1
    ...
    n_transformations = len(powers.keys()) if BoxCox else len(powers.keys()) + 1 
