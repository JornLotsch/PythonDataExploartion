...
def determine_n_dims(ndimensions, ndimensionsmethod, model, reconstructionError, reconstructionError_dists, indexGE90):
...
def identify_relevant_variables_ABC(reconstruction_error_per_variable_values, reconstruction_error_per_variable_dist, feature_imp):
    ...
    relevant_variables_reconstruction_error_values = \
        ABC_analysis(reconstruction_error_per_variable_values.replace([np.inf, -np.inf], np.nan, inplace=False).dropna().sum() -
                     reconstruction_error_per_variable_values)["Aind"]

    relevant_variables_reconstruction_error_dists = \
        ABC_analysis(reconstruction_error_per_variable_dist.replace([np.inf, -np.inf], np.nan, inplace=False).dropna().sum() -
...
def perform_projections(data, y1=None, y2=None, scale=True, showonlyPC12=False, biplot=True, ndimensions=None, minexplainedvarperc=90,
    ...
    n_dims = determine_n_dims(ndimensions=ndimensions, ndimensionsmethod=ndimensionsmethod, model=model_projection,
                              reconstructionError=reconstructionError, reconstructionError_dists=reconstructionError_dists,
                              indexGE90=indexGE90)
    idxn_dim = n_dims - 1
    ...
    return {"Fig": results_grid, "loadings": loadings_df, "projections": finalDf, 
            ...
            "relevant_features_reconstr_values": relevant_variables_reconstruction_error_values, 
            "relevant_features_reconstr_dists": relevant_variables_reconstruction_error_dists}
