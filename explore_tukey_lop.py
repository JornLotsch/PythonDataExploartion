...
def create_plots(data, powers, normtest):
    ...
    fig, axes = plt.subplots(n_transformations, 3, figsize=(20, 5*n_transformations))
    for i, actual_power in enumerate(list(powers.values())):
        data_transformed, BClambdaX = data_transform(data=data, tk_power=actual_power)
        if actual_power == 10:
            BClambda = BClambdaX
        data_transformed = data_transformed[~np.isnan(data_transformed)]
        data_transformed = data_transformed[~np.isinf(data_transformed)]
       
        figColors =[("purple" if i == round(BClambda,0) else ("salmon" if i == 1 else "dodgerblue")) for i in powers.values()]
        ...
        if len(powers.keys()) > 1:
            ...
            pdf_title = "pdf: transformation = " + list(powers.keys())[i] if actual_power != 10 else \
                "pdf: transformation = " + list(powers.keys())[i] + ", lambda = " + "{:.2f}".format(round(BClambda, 2)) 
...
def explore_tukey_lop(data, powers=[-3, -2, -1, -0.5, -0.333333, 0, 0.333333, 0.5, 1, 2, 3], normtest="K^2 test", outlierremoval=False):
    ...
    if dataOK: 
        figure = create_plots(data=CleanedData, powers=powers, normtest=normtest)
