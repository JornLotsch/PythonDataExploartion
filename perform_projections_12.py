...
from plot_PCAresults_grid_4 import plot_results_grid
...
def compute_reconstructionError(data_sc, n_dims):

    model_projection_reconstruction = PCA(n_components=n_dims)
    projections_reconstruction = model_projection_reconstruction.fit_transform(data_sc)
    data_reconstructed = model_projection_reconstruction.inverse_transform(projections_reconstruction)
    
    mean_sq_reconstructionError = np.sqrt(
            ((data_reconstructed - data_sc) ** 2).mean().mean()) / data_sc.to_numpy().ravel().std()

    data_sc_dist = distance_matrix(data_sc, data_sc)
    data_reconstructed_dist = distance_matrix(data_reconstructed, data_reconstructed)
    
    if (data_reconstructed_dist - data_sc_dist != 0).all:
        mean_sq_reconstructionError_dist = np.sqrt(((data_reconstructed_dist - data_sc_dist)
                                                          ** 2).mean())/data_sc_dist.std()
    else:
        mean_sq_reconstructionError_dist = 0

    return mean_sq_reconstructionError, mean_sq_reconstructionError_dist, data_reconstructed, data_sc_dist, data_reconstructed_dist
...
def perform_projections(data, y1=None, y2=None, scale=True, showonlyPC12=False, biplot=True, ndimensions=None, minexplainedvarperc=90,
    ...
    implemented_ndimensionsmethods = ["KaiserGuttman", "ABCeigenvalues", "ExplainedVariance"]
    ...
    reconstructionError, reconstructionError_dists, data_reconstructed, data_sc_dist, \
        data_reconstructed_dist = compute_reconstructionError(data_sc=X_sc, n_dims = n_dims)
    ...
    results_grid = plot_results_grid(ndimensionsmethod=ndimensionsmethod, showPCs=showPCs, finalDf=finalDf, scale=scale, biplot=biplot,
                                     ...
                                     X_reconstructed=data_reconstructed, X_reconstructed_dists=data_reconstructed_dist,
    ...
    return {"Fig": results_grid, "loadings": loadings_df, "projections": finalDf, 
            "reconstructions": data_reconstructed, 
